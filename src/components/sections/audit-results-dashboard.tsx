...
const categorizeIssue = (checkType: string, count: number) => {
  ...
  const goodIssues: Record<string, { title: string; description: string; explanation: string }> = {
    is_https: {
      title: 'Conexión Segura',
      description: 'Tu sitio usa HTTPS (conexión segura)',
      explanation: 'Google prefiere sitios seguros y los usuarios confían más en ellos.'
    },
    has_html_doctype: {
      title: 'Código HTML Correcto',
      description: 'Tu sitio tiene el código HTML bien estructurado',
      explanation: 'Esto ayuda a que los navegadores muestren tu sitio correctamente.'
    },
    canonical: {
      title: 'URLs Bien Configuradas',
      description: 'Google sabe cuál es la versión oficial de cada página',
      explanation: 'Evita que Google indexe páginas duplicadas y mejora tu posicionamiento.'
    }
  };

  // Problemas de MEJORA (amarillo)
  const improvementIssues: Record<string, { title: string; description: string; explanation: string }> = {
    low_content_rate: {
      title: 'Poco Contenido en Algunas Páginas',
      description: 'Algunas páginas tienen poco texto',
      explanation: 'Google prefiere páginas con más contenido útil para los usuarios.'
    },
    no_image_title: {
      title: 'Faltan Títulos en Algunas Imágenes',
      description: 'Algunas imágenes no tienen títulos descriptivos',
      explanation: 'Los títulos ayudan a Google a entender qué hay en las imágenes.'
    },
    ...
    seo_friendly_url_keywords_check: {
      ...
      explanation: 'Incluir palabras clave en la URL ayuda con el posicionamiento.'
    },
    ...
    title_too_short: {
      ...
      explanation: 'Títulos más largos (50-60 caracteres) son mejores para SEO.'
    },
    low_character_count: {
  ...
  const realIssues: Record<string, { title: string; description: string; explanation: string }> = {
    has_render_blocking_resources: {
      ...
      explanation: 'Esto molesta a los usuarios y Google penaliza sitios lentos.'
    },
    no_h1_tag: {
      title: 'Faltan Títulos Principales',
      description: 'Algunas páginas no tienen títulos principales (H1)',
      explanation: 'Los títulos H1 le dicen a Google cuál es el tema principal de la página.'
    },
    irrelevant_meta_keywords: {
      title: 'Palabras Clave Irrelevantes',
      description: 'Algunas páginas tienen palabras clave que no coinciden con el contenido',
      explanation: 'Google puede penalizar sitios que usan palabras clave engañosas.'
    }
  };

  if (goodIssues[checkType]) {
    return { ...goodIssues[checkType], type: 'good', icon: CheckCircle2, color: 'text-green-600', badgeColor: 'bg-green-100 text-green-800 border-green-200 hover:bg-green-600 hover:text-white' };
  }

  if (improvementIssues[checkType]) {
    return { ...improvementIssues[checkType], type: 'improvement', icon: AlertTriangle, color: 'text-yellow-600', badgeColor: 'bg-yellow-100 text-yellow-800 border-yellow-200 hover:bg-yellow-600 hover:text-white' };
  }

  if (realIssues[checkType]) {
    return { ...realIssues[checkType], type: 'problem', icon: AlertCircle, color: 'text-red-600', badgeColor: 'bg-red-100 text-red-800 border-red-200 hover:bg-red-600 hover:text-white' };
  }

  // Fallback para problemas no categorizados
  return {
    title: checkType.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    ...
    icon: Info,
    color: 'text-gray-600',
    badgeColor: 'bg-gray-100 text-gray-800 border-gray-200 hover:bg-gray-600 hover:text-white'
  };
};
          ...
          <CardContent className="flex items-center gap-3 pt-6">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">
                Páginas analizadas: {summary.crawl_status?.pages_crawled || 0} de{' '}
                {summary.crawl_status?.max_crawl_pages || 100}
              </p>
        ...
        <CardContent>
          <div className="flex items-center gap-8">
            ...
            <div className="flex-1 space-y-2">
              ...
              <p className="text-muted-foreground text-sm">
                Basado en el análisis de {summary.page_metrics ?
                  (summary.page_metrics.links_internal + summary.page_metrics.links_external) : 0} elementos
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {summary.page_metrics?.checks && Object.keys(summary.page_metrics.checks).length > 0 && (
        <Card>
          <CardHeader>
            ...
            <p className="text-muted-foreground text-sm">
              Aquí tienes un resumen de lo que está bien y lo que se puede mejorar
            </p>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              {(() => {
                const issues = Object.entries(summary.page_metrics.checks).filter(([_, count]) => count > 0);
                const goodCount = issues.filter(([type]) => categorizeIssue(type, 0).type === 'good').length;
                const improvementCount = issues.filter(([type]) => categorizeIssue(type, 0).type === 'improvement').length;
                const problemCount = issues.filter(([type]) => categorizeIssue(type, 0).type === 'problem').length;
                ...
                return (
                  <>
                    <div className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4">
                      ...
                      <div>
                        <p className="font-semibold text-green-900">{goodCount}</p>
                        <p className="text-sm text-green-700">Cosas que están bien</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                      <AlertTriangle className="size-6 text-yellow-600" />
                      <div>
                        <p className="font-semibold text-yellow-900">{improvementCount}</p>
                        <p className="text-sm text-yellow-700">Se pueden mejorar</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
                      <AlertCircle className="size-6 text-red-600" />
                      <div>
                        <p className="font-semibold text-red-900">{problemCount}</p>
                        <p className="text-sm text-red-700">Necesitan atención</p>
                      </div>
                    </div>
                  </>
                );
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="text-muted-foreground">{summary.domain_info.name}</p>
              </div>
              {summary.domain_info.cms && (
                <div>
                  <p className="text-sm font-medium">CMS Detectado</p>
                  <p className="text-muted-foreground">{summary.domain_info.cms}</p>
                </div>
              )}
              {summary.domain_info.ip && (
                <div>
                  <p className="text-sm font-medium">IP del Servidor</p>
                  <p className="text-muted-foreground">{summary.domain_info.ip}</p>
                </div>
              )}
              ...
              {summary.domain_info.ssl_info?.valid_certificate !== undefined && (
                <div>
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-blue-700 mt-2 mb-4">
                Nuestro equipo puede mejorar tu sitio web y hacer que aparezca mejor en Google.
                Te explicamos todo paso a paso y trabajamos contigo.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <a
                  href="/pricing"
                  className="inline-block rounded-lg border border-blue-600 px-6 py-3 font-medium text-blue-600 hover:bg-blue-600 hover:text-white transition-colors"
                >
                  Ver Nuestros Planes
                </a>
              </div>
