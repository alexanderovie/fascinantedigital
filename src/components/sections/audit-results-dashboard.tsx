...
const categorizeIssue = (checkType: string, count: number) => {
  ...
  if (goodIssues[checkType]) {
    return { ...goodIssues[checkType], type: 'good', icon: CheckCircle2, color: 'text-green-600', badgeColor: 'bg-green-100 text-green-800 border-green-200' };
  }

  if (improvementIssues[checkType]) {
    return { ...improvementIssues[checkType], type: 'improvement', icon: AlertTriangle, color: 'text-yellow-600', badgeColor: 'bg-yellow-100 text-yellow-800 border-yellow-200' };
  }

  if (realIssues[checkType]) {
    return { ...realIssues[checkType], type: 'problem', icon: AlertCircle, color: 'text-red-600', badgeColor: 'bg-red-100 text-red-800 border-red-200' };
  }
              ...
              {Object.entries(summary.page_metrics.checks)
                ...
                .map(([checkType, count]) => {
                  ...
                  return (
                    <div
                      key={checkType}
                      className={`rounded-lg border p-4 ${issueInfo.type === 'good' ? 'border-green-200 bg-green-50' :
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <IconComponent className={`size-5 mt-0.5 ${issueInfo.color}`} />
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-2">
                            <h3 className="font-semibold text-gray-900">
                              {issueInfo.title}
                            </h3>
                            <Badge className={issueInfo.badgeColor}>
